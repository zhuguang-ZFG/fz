...
def write_wokwi_toml(firmware: Path, elf: Optional[Path], out_dir: Path) -> Path:
    ...
    # copy or link-style: write absolute paths in toml (Windows-friendly)
    fw = firmware.resolve()
    el = elf.resolve() if elf and elf.is_file() else fw.with_suffix(".elf")
    text = f"""# auto-generated by run_wokwi_smoke.py — do not commit secrets
# Espressif: https://docs.espressif.com/projects/esp-idf/zh_CN/latest/esp32s3/third-party-tools/wokwi.html

[wokwi]
version = 1
firmware = '{fw.as_posix()}'
elf = '{el.as_posix()}'
